...
router.post("/add", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let car = null;
        if (req.body.event &&
            ((_a = req.body.event) === null || _a === void 0 ? void 0 : _a.trim()) !== "" &&
            req.body.event !== "user") {
            yield (0, star_message_service_1.triggerEvent)(req.body.event, {
                data: userMessage,
            }, user.id, source);
        }
        else {
            ...
            const recentMessages = yield (0, star_message_service_1.getRecentMessages)(user.id);
            if (recentMessages.length === 0) {
                yield (0, star_message_service_1.initConversation)(user, (car === null || car === void 0 ? void 0 : car.color) || "Unknown", source);
            }
        ...
        const freshMessages = yield (0, star_message_service_1.getRecentMessages)(user.id);
        ...
        const systemMessageContent = yield (0, generate_initial_star_instruction_1.generateInitialStarInstruction)(user, (car === null || car === void 0 ? void 0 : car.color) || "Unknown");
        const systemMessage = {
            role: "system",
        ...
        const openResponse = yield (0, star_message_service_1.getOpenAIResponse)(reversedMessages);
        ...
        if (toolCall) {
            yield (0, star_message_service_1.handleToolCalls)(toolCall, user.id, source);
            for (let i = 0; i < toolCall.length; i++) {
                const tool = toolCall[i];
                toolResponse = yield (0, star_message_service_1.parseToolCall)(tool, user.id, source);
            }
            const messages = yield (0, star_message_service_1.getRecentMessages)(user.id);
            const reversed = messages.reverse();
            const response = yield (0, star_message_service_1.getOpenAIResponse)(reversed.map((message) => message.content));
...
router.post("/init", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // add an else condition to detect the latest car driven by the user
        // it's going to be unknown if the source is "app"
        let recentMessages = yield (0, star_message_service_1.getRecentMessages)(user.id);
        if (recentMessages.length !== 0) {
            if (recentMessages[0].content.tool_calls) {
                yield (0, star_message_service_1.handleToolCalls)(recentMessages[0].content.tool_calls, user.id, source);
                for (let i = 0; i < recentMessages[0].content.tool_calls.length; i++) {
                    const tool = recentMessages[0].content.tool_calls[i];
                    yield (0, star_message_service_1.parseToolCall)(tool, user.id, source);
                }
            }
            yield (0, star_message_service_1.triggerEvent)("app", {
                userDetected: true,
                description: "This means that the car has detected the user and the auxillary battery systems have been powered on along with the OS. You can use this as a cue to initiate conversation with the user.",
            }, user.id, source);
            recentMessages = yield (0, star_message_service_1.getRecentMessages)(user.id);
